...
import {
  ...
  getDocs, onSnapshot, setDoc, deleteDoc
...
const AdminPanel = () => {
  ...
  useEffect(() => {
    if (isAdmin && videoRef.current && !scannerRef.current) {
      ...
      scannerRef.current.start().catch((err) => {
        ...
        alert("Camera could not start. Check permissions or if another app is using it.");
      });
    }
  ...
  }, [isAdmin]); // Only depends on Admin status
  ...
  // --- DATA LISTENERS ---
  useEffect(() => {
    const unsubStd = onSnapshot(doc(db, "settings", "pricing"), (docSnap) => { if (docSnap.exists()) setStandardPricing(docSnap.data()); });
    const unsubPeak = onSnapshot(doc(db, "settings", "peak"), (docSnap) => { if (docSnap.exists()) setPeakPricing(docSnap.data()); });
  ...
  const confirmPayment = async (method) => {
    ...
    setIsUpdating(true);
    const userBonus = !userData.isGuest && userData.rewardClaimed === false;
    const friendBonus = friendData && friendData.rewardClaimed === false;
    const hasBonus = userBonus || friendBonus;
    
    const finalDuration = hasBonus ? (pendingTransaction.hours + 0.25) : pendingTransaction.hours;
    ...
    try {
      ...
      else await updateDoc(userRef, { sessions: arrayUnion(sessionData), rewardClaimed: userBonus ? true : userData.rewardClaimed });

      if (friendData) {
        const friendRef = doc(db, "users", friendData.uid);
        await updateDoc(friendRef, { sessions: arrayUnion(sessionData), rewardClaimed: friendBonus ? true : friendData.rewardClaimed });
      }

      setReceiptData({ ...sessionData, name: playersNames, total: finalAmount });
      setTimeout(() => {
        window.print();
      ...
      <div id="receipt-print" style={{ display: "none" }}>
        {receiptData && (
          <div style={{ width: "75mm", padding: "10px", color: "black", fontFamily: "monospace" }}>
            ...
            <p><b>TIME:</b> {receiptData.duration} Hrs</p>
            ...
            <div className="box mb-4" style={{ background: "#1a1a1a", border: "1px solid #333" }}>
              ...
              {activeSessions.map((s, i) => (
                <div key={i} className="mb-2 p-2" style={{ background: "#222", borderLeft: s.timeLeft <= 5 ? "4px solid #ff3860" : "4px solid #00d1b2" }}>
                  ...
                  <p className={`is-size-7 ${s.timeLeft <= 5 ? 'has-text-danger has-text-weight-bold' : 'has-text-warning'}`}>{s.timeLeft}m left</p>
                </div>
              ))}
            </div>
              ...
              <div className="box" style={{ background: "#111", border: "1px solid #00d1b2" }}>
                <div className="columns">
                  <div className="column">
                    ...
                    <div className="field has-addons mt-4">
                      <div className="control is-expanded"><input className="input is-small is-dark" placeholder="Add Friend Phone (Optional)..." value={friendPhone} onChange={(e) => setFriendPhone(e.target.value)} /></div>
                ...
                <div className="columns mt-4">
                  <div className="column">
                    ...
                    {MACHINES.map(m => {
                      const isOcc = activeSessions.some(s => s.machine === m);
                      return (
                        <button key={m} disabled={isOcc} className={`button is-small is-fullwidth mb-2 ${selectedMachine === m ? "is-primary" : "is-dark"}`} onClick={() => setSelectedMachine(m)}>
                          {m} {isOcc ? "(In Use)" : ""}
                        </button>
                      );
                    })}
                ...
                {pendingTransaction && (
                  <div className="notification mt-4 is-dark" style={{ border: "1px solid #00d1b2" }}>
                    ...
                    <div className="field">
                      <input className="input is-small is-dark" placeholder="Record Lap Time (Optional)" value={lapTime} onChange={(e) => setLapTime(e.target.value)} />
                    </div>
